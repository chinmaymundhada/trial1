...
return await step.prompt(CONFIRM_PROMPT, 'For which shivir would you like to register ?', ['Maha Asmani', 'Sampurna Swastha']);
...
async getName(step){
	...
	step.values.Step = step.result
    if(step.result === true)
    { 
    return await step.prompt(TEXT_PROMPT, 'https://tgf.tejgyan.org/mahaaasmaniregistration.jsp');
    }

    if(step.result === false)
    { 
        await step.context.sendActivity("https://tgf.tejgyan.org/healthRegistration.jsp.");
