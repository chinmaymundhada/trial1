...
class RRBOT extends ActivityHandler {
    ...
    async sendWelcomeMessage(turnContext) {
        ...
        for (const idx in activity.membersAdded) {
            if (activity.membersAdded[idx].id !== activity.recipient.id) {
                const welcomeMessage = `Happy Thoughts ! ${ activity.membersAdded[idx].name }. `;
    ...
    async sendSuggestedActions(turnContext) {
        var reply = MessageFactory.suggestedActions(['Register for shivir','Guidance regarding Shivir'],'What can I help you with ?');
    ...
    switch(currentIntent)
    {
        ...
        case 'Guidance regarding Shivir':
